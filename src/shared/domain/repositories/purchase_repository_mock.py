...
class PurchaseRepositoryMock(IChallengeRepository):
    ...
    def __init__(self):
        self.purchase_list = [
            ...
            Purchase(user_id="3c0c67a5-2dc4-4b90-be13-73b7d29718f0", product_id="7aa45600-3e44-46ca-8064-aa90874288cf", 
                        category=CATEGORY.CLOTHES, price= 25.0, purchase_date= 1744644600), # 1
            Purchase(user_id="3c0c67a5-2dc4-4b90-be13-73b7d29718f0", product_id="7aa45600-3e44-46ca-8064-aa90874288cf", 
                        category=CATEGORY.CLOTHES, price= 50.0, purchase_date= 1744644600), # 2
            Purchase(user_id="3c0c67a5-2dc4-4b90-be13-73b7d29718f0", product_id="7aa45600-3e44-46ca-8064-aa90874288cf", 
                        category=CATEGORY.CLOTHES, price= 75.0, purchase_date= 1744644600), # 3
            Purchase(user_id="3c0c67a5-2dc4-4b90-be13-73b7d29718f0", product_id="7aa45600-3e44-46ca-8064-aa90874288cf", 
                        category=CATEGORY.ELECTRONIC, price= 100.0, purchase_date= 1744644600), # 4
    ...
    def get_purchase_from_user_id_and_interval(self, user_id: str, start_date: int, end_date: int) -> Optional[Dict]:
        ...
        total_spent = 0.0
        category_breakdown = defaultdict(lambda: {"amount_spent": 0.0, "count": 0})

        for purchase in filtered_purchases:
            # Usando o atributo 'price' ao invés de 'amount'
            amount = purchase.price
            category = purchase.category.value
            date = purchase.purchase_date

            total_spent += amount
            category_breakdown[category]["amount_spent"] += amount
